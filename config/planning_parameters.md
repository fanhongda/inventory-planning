...
# 数量取整。计件物料的预测、安全库存、再订货点不该带小数 —— 33.4 个不是一个预测,
# 12.4 的安全库存也不是一个采购员能下的数。
#   integer  四舍五入到整数,除非该 SKU 的计量单位明确是连续计量
#   none     保留一位小数(此约定出现之前的行为)
#
# 注意判定的方向:**除非单位说它是连续计量,否则取整**,而不是"单位说它是件才取整"。
# 后者在真实导出上一次都不会触发 —— 83 列的主数据、库存导出、销售历史三者都不带
# 计量单位。没有单位的 SKU 一律按计件处理,这是一条假设,运行末尾会报出它覆盖了多少个数字。
#
# 四舍五入而不是向上取整:lot_sizing 那里向上是对的(EOQ 在最优点附近很平,少订要多付
# 一次订货成本),这里没有那种不对称 —— 预测是对数量的估计而不是对数量的决定,全部向上
# 会把系统性偏差塞进每一个下游数字。
quantity_rounding: integer

# 计量而非计数的单位。刻意列得短而明确:不在表里的一律按计件处理,所以一个不认识的
# 代码会偏向整数,而不是偏向一个没人能下单的小数。
continuous_uom: [KG, G, MG, T, TO, TON, LB, OZ,
                 L, ML, CL, GAL, M3, CBM,
